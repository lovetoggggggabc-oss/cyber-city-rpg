...
const camForward={x:0,y:0,z:0},camRight={x:0,y:0,z:0},camUp={x:0,y:0,z:0};
let eye={x:0,y:53,z:0},focal=1;
function camera3D(){eye.x=p.x;eye.z=p.y;
 const a=p.face,c=Math.cos(lookPitch),s=Math.sin(lookPitch);
 camForward.x=Math.cos(a)*c;camForward.y=s;camForward.z=Math.sin(a)*c;
 camRight.x=-Math.sin(a);camRight.y=0;camRight.z=Math.cos(a);
 camUp.x=-Math.cos(a)*s;camUp.y=c;camUp.z=-Math.sin(a)*s;
 focal=Math.max(W*.77,H*.83)}
function depth3(v){return(v[0]-eye.x)*camForward.x+(v[1]-eye.y)*camForward.y+(v[2]-eye.z)*camForward.z}
function project(x,y,z){let dx=x-eye.x,dy=y-eye.y,dz=z-eye.z,depth=dx*camForward.x+dy*camForward.y+dz*camForward.z;
 if(depth<8)return null;
 return{x:W/2+(dx*camRight.x+dz*camRight.z)*focal/depth,y:H/2-(dx*camUp.x+dy*camUp.y+dz*camUp.z)*focal/depth,depth}}
function clipNear(points){let result=[];for(let i=0;i<points.length;i++){let a=points[i],b=points[(i+1)%points.length],da=depth3(a),db=depth3(b),insideA=da>=8,insideB=db>=8;
 if(insideA)result.push(a);if(insideA!==insideB){let t=(8-da)/(db-da);result.push(a.map((v,k)=>v+(b[k]-v)*t))}}return result}
function polygon(points,fill,stroke){let clipped=clipNear(points);if(clipped.length<3)return;let pr=clipped.map(v=>project(...v));if(pr.some(v=>!v))return;
 ctx.beginPath();ctx.moveTo(pr[0].x,pr[0].y);for(let i=1;i<pr.length;i++)ctx.lineTo(pr[i].x,pr[i].y);ctx.closePath();ctx.fillStyle=fill;ctx.fill();if(stroke){ctx.strokeStyle=stroke;ctx.lineWidth=1;ctx.stroke()}}
const surfaces=[];
function cube3(x,z,w,d,h,colors,base=0,alpha=1){let x0=x-w/2,x1=x+w/2,z0=z-d/2,z1=z+d/2,y0=base,y1=base+h;
 let faces=[[[x0,y1,z0],[x1,y1,z0],[x1,y1,z1],[x0,y1,z1]],
 [[x0,y0,z0],[x0,y0,z1],[x0,y1,z1],[x0,y1,z0]],
 [[x1,y0,z0],[x1,y0,z1],[x1,y1,z1],[x1,y1,z0]],
 [[x0,y0,z0],[x1,y0,z0],[x1,y1,z0],[x0,y1,z0]],
 [[x0,y0,z1],[x1,y0,z1],[x1,y1,z1],[x0,y1,z1]]];
 for(let i=0;i<faces.length;i++){let pts=faces[i],dists=pts.map(depth3);if(dists.every(v=>v<8))continue;surfaces.push({pts,depth:dists.reduce((s,v)=>s+v,0)/4,color:colors[i%3],stroke:i===0?'#83dcf451':'#121f30',alpha})}}
function line3(a,b,color,width=1){let da=depth3(a),db=depth3(b);if(da<8&&db<8)return;if(da<8||db<8){let t=(8-da)/(db-da),v=a.map((x,k)=>x+(b[k]-x)*t);if(da<8)a=v;else b=v}let p1=project(...a),p2=project(...b);if(!p1||!p2)return;ctx.strokeStyle=color;ctx.lineWidth=width;ctx.beginPath();ctx.moveTo(p1.x,p1.y);ctx.lineTo(p2.x,p2.y);ctx.stroke()}
function paintWorld(){let bg=ctx.createLinearGradient(0,0,0,H);bg.addColorStop(0,'#071120');bg.addColorStop(1,'#28344c');ctx.fillStyle=bg;ctx.fillRect(0,0,W,H);
 ...
 for(let i=0;i<world.h/120;i++){let z=i*120;line3([400,.7,z],[400,.7,z+75],'#49cde6',2);line3([1050,.7,z],[1050,.7,z+75],'#e36aaf',2)}
